"""Emotional anomaly detector for V9 engine -- orbital analysis.

Finds emotional anomalies by looking at the ORBIT of the words,
not just the words themselves. Takes per-message VADUG scores
directly (no pipeline dependency) and maintains its own conversation
history.
...
Usage:
    from engine_v9.anomaly import AnomalyDetector
    from engine_v9.shared import VADUG

    detector = AnomalyDetector()
    for msg in messages:
        vadug = VADUG(v=120, a=130, d=100, u=10, g=110, w=115, i=128)
        result = detector.process_message(msg, vadug)
        if result.anomalies:
...
class AnomalyDetector:
    ...
    Accepts per-message VADUG scores directly. Maintains conversation
    ...
    def process_message(
        ...
        Args:
            ...
            vadug: pre-computed VADUG score
            meta: optional trace dict -- may contain "trace" list with per-word
                  entries (role, word, v) for identifying emotional word firings

        Returns an AnomalyResult containing the VADUG score plus
    ...
    def _build_record(self, text: str, vadug: VADUG, meta: dict) -> _TurnRecord:
        ...
        # Check trace for emotional word firings (uses "EMOTIONAL" role)
        emotional_words = []
    ...
    def _detect_deflection(self, current: _TurnRecord) -> List[Anomaly]:
        ...
        for em_turn in emotional_turns:
            ...
            # Withdrawing intent amplifies deflection signal
            if is_withdrawing:
